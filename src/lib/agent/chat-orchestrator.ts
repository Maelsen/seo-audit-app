...
function buildSystemPrompt(context: ChatContext): string {
  ...
  parts.push(
    ...
    "",
    "   WICHTIG - zwei Zustaende einer Seite im PDF-Template:",
    "   A) NICHT AUFGETEILT: Die Seite enthaelt EINEN Block vom type 'legacyPage'. Der rendert die React-Komponente aus src/components/pdf-template/pages/*.tsx. -> Um Text/Style zu aendern: React-Komponente editieren.",
    "   B) AUFGETEILT (User hat 'Elemente bearbeiten' geklickt): Die Seite enthaelt MEHRERE Bloecke (textBlock, imageBlock, shapeBlock, ...). Jeder Block hat seinen eigenen Content+Style direkt im Template-JSON. -> Um Text zu aendern: den Block im JSON unter data/templates/{templateId}.json editieren (Feld 'text' beim textBlock, 'src' beim imageBlock usw). React-Komponenten aus pdf-template/ werden hier NICHT mehr verwendet!",
    "",
    "   Um zu wissen in welchem Zustand die Seite ist: lies data/templates/{templateId}.json, suche die Seite mit der selectedPageId. Hat sie nur einen Block vom type 'legacyPage' -> Fall A. Hat sie mehrere Bloecke -> Fall B.",
    "",
    "   Wenn context.selectedBlockId gesetzt ist, ist der User in Fall B auf einem konkreten Block. Edit ausschliesslich diesen Block im Template-JSON.",
    "",
    "Entscheidungs-Regel:",
