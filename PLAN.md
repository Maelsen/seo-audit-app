...
- [x] M7 UX & Conversion (2 pages) — `buildUxConversion1()` (Score-Donut + bound-Sub-Headline/Text + findingsTable mit 12 Rows) und `buildUxConversion2()` (Cost-Heading + Body + Action-Heading + arrowBulletList 6 Pfeil-Items + closingNote) als Subset von M6's Pattern (kein SerpPreview, kein BarChart, nur 1 Footer-Note). Default-Template hat 162 Blocks (+12 vs M6). seed-vasileios-audit M7-Daten ergaenzt mit allen 12 findings + 6 actions + closingNote.
