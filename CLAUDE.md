...
- **`/verify-chrome-editor-e2e <templateId> [auditId]`** — Klickt im Chrome-Browser jeden Block eines Templates an, liest dessen Inspector-Werte und vergleicht sie mit der Backend-JSON. Pruefe automatisch: pro Block ✓/✗, Drag-Sanity, Save+Reload-Persistenz, Console-Errors. Wraps die `claude-in-chrome` MCP-Sequenz. **Mit clickBlock-Helper (M8 hinzugefuegt):** ein JS-Snippet liest `getBoundingClientRect()` + `scrollIntoView()` aus dem DOM und gibt Viewport-Pixel-Mitte zurueck — `computer.left_click` an diese Koords ist der einzige stabile Klick-Pfad (synthetic dispatchEvent geht am Editor-Pointer-Layer vorbei). Inspector-Volltext-Reader (`{blockHeader, selects[]}`) liest letzten select-Eintrag als Binding-Label — zweite Verteidigungslinie gegen den M7-closingNote-Bug. Wann nutzen: nach jedem Page-Builder-Update in M4-M13 BEVOR der Builder als "fertig" abgehakt wird. Definiert in `.claude/skills/verify-chrome-editor-e2e/SKILL.md`.
