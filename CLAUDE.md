...
Sechzehn Automation-Artefakte unter `.claude/` und `scripts/`. Alle committed, beim naechsten Session-Start automatisch verfuegbar.
...
- **`/diff-text-rows-vs-vasileios <auditId> <pageNum> [templateId] [xMin] [xMax]`** — Quantifiziert mm-Drift App-PDF vs Vasileios-Referenz-PNG fuer EINE Page durch white-text-row-Detection. Output: Drift-Tabelle pro Text-Row mit ✓/⚠/✗ Flags + Summary. Schneller als der pdf-verifier Subagent (~5s vs 30s) und gibt konkrete mm-Werte. xMin/xMax (Pixel) grenzen den Spalten-Scan ein — fuer 2-Spalten-Layouts wie M13 Page 20 z.B. `160 700` (LEFT) oder `1100 1500` (RIGHT). Wann nutzen: nach jedem Page-Builder-Update zur quantitativen Drift-Pruefung, nach lineHeight/fontSize-Tuning um Drift vor/nach zu vergleichen, beim Bauen neuer Pages in M14+ wenn die Migration sich erweitert. Caveat: detektiert nur **white** text-rows (R/G/B > 200), cyan-Texte werden uebersehen. Bei row-count-mismatch zwischen App und Referenz: meist seed-Audit-Texte != Vasileios-Original-Texte (Wort-Anzahl unterschiedlich), nicht echte Drift. Definiert in `.claude/skills/diff-text-rows-vs-vasileios/SKILL.md`. **In M13 entstanden** — ich hab dort 4× den gleichen Inline-Python-Block geschrieben fuer App-vs-Vasileios-Drift-Pruefung.

- **`/measure-vasileios-page <pageNum> [element] [yMin] [yMax]`** — Vermisst Layout-Elemente in `docs/measurements/page-NN.png` per Python+PIL und gibt mm-Bbox + Hex-Color aus. Element ist `logo` | `header-text` | `footer-stripe` (Standard-Chrome, default-Range full-page) oder `text-rows` | `pills` | `dividers` | `cyan-region` | `image-regions` (Custom-Layout, brauchen yMin/yMax in mm) oder `all` (alle Chrome). Wann nutzen: VOR jedem Page-Builder in M4-M13. Standard-Chrome: `5 logo`. Custom-Layout fuer M5+: `4 pills 155 175` fuer Tabellen-Pills, `4 dividers 165 285` fuer Row-Dividers, **`10 image-regions 95 185` fuer Image-Slot-Frames (in M8 hinzugefuegt — detect dunkle Card-Bereiche per lum-Threshold > 49, gut fuer Pages mit eingebetteten Bildern wie M8/M9 Schema-Markup-Bild)**. Definiert in `.claude/skills/measure-vasileios-page/SKILL.md`.

- **`/seed-edge-case-audit <baseAuditId> [milestone]`** — Erzeugt ein Edge-Case-Audit aus einem Base-Audit mit allen milestone-relevanten Arrays geleert (M5: topRisks, comparison.altSentences, comparison.rows; M6: onpageSeo.findings/actions; **M7 (vollstaendig): uxConversion.findings/actions/heading/text/costText/closingNote**; **M8 (vollstaendig): seitenstrukturContent.findings/actions/comparisonImages/heading/text/costText/closingNote** — alle bound Felder; M9-M13 aktuell nur findings/actions, bei Bedarf analog M7/M8 erweitern). Output unter `data/audits/<base>-empty-<milestone>.json`. Wann nutzen: VOR `/verify-feature` am Milestone-Ende fuer Empty-State-Crash-Test. milestone ist `M5|M6|M7|M8|M9|M10|M11|M12|M13|all`. Definiert in `.claude/skills/seed-edge-case-audit/SKILL.md`.

- **`/seed-vasileios-audit <auditId> [milestone]`** — Schreibt ein Audit mit den exakten Vasileios Waschbaer-Service-Texten pro Milestone (aktuell **M5 + M6 + M7 + M8** vollstaendig erfasst, M9-M13 als Stubs). Output unter `data/audits/<auditId>.json`. **Skill ruft jetzt `scripts/seed-vasileios-audit.ts` auf** (in M8 refactored — der frueher inline Python-Block hatte `$1`/`$2` Args-Resolver-Bug, deterministische `process.argv` per TS fixen). Wann nutzen: VOR `/visual-diff-against-vasileios` damit App-PDF die gleichen Texte zeigt wie Referenz, oder als Test-Audit fuer `/verify-feature`. Wenn neue Milestones gebaut werden: `DATA` Object in `scripts/seed-vasileios-audit.ts` ergaenzen + Tabelle in der SKILL.md. Definiert in `.claude/skills/seed-vasileios-audit/SKILL.md` (+ `scripts/seed-vasileios-audit.ts`).

- **`/verify-chrome-editor-e2e <templateId> [auditId]`** — Klickt im Chrome-Browser jeden Block eines Templates an, liest dessen Inspector-Werte und vergleicht sie mit der Backend-JSON. Pruefe automatisch: pro Block ✓/✗, Drag-Sanity, Save+Reload-Persistenz, Console-Errors. Wraps die `claude-in-chrome` MCP-Sequenz. **Zwei Klick-Pfade (M10 erweitert):** **Stage-0 (JS-only Sweep)** dispatcht synthetisch PointerEvents an `[data-overlay-block-id]`-Mitte (~700ms/Block, ideal fuer Inspector-Bindings-Sweep auf 20+ Blocks/Page). Akzeptiert die `setPointerCapture`-NotFoundError-Exception als Test-Tooling-Artefakt — Selektion klappt trotzdem, Inspector aktualisiert sich, und echter User-Klick mit Maus loest die Exception nicht aus. **Stage-1 (echter Pixel-Klick)** liest `getBoundingClientRect()` + `scrollIntoView()` aus dem DOM und gibt Viewport-Pixel-Mitte zurueck — `computer.left_click` an diese Koords noetig fuer Drag/Move/Resize-Tests (dispatchEvent crasht da im React-Pointer-Capture-Path). Inspector-Reader liest letzten `<select option:checked>` als Binding-Label — zweite Verteidigungslinie gegen den M7-closingNote-Bug. Console-Filter via `jq` schliesst `setPointerCapture` aus. **Bekannte Editor-Limitation (M10 entdeckt):** `gauge`/`resourceTile`/`pieChart`/`barChart`-Blocks rendern keinen Binding-Selector im Inspector — `selects: []` ist dort erwartet, kein Bug. Wann nutzen: nach jedem Page-Builder-Update in M4-M13 BEVOR der Builder als "fertig" abgehakt wird. Definiert in `.claude/skills/verify-chrome-editor-e2e/SKILL.md`.
...
- **`binding-catalog-consistency` Hook** (auto-aktiv, kein Aufruf) — PostToolUse-Hook der nach Edit/Write auf `src/lib/editor/page-builders.ts` alle `binding: { kind: "audit", path: "..." }` extrahiert und prueft dass jeder Path im `BINDING_CATALOG` (`src/lib/editor/binding-catalog.ts`) steht. Faengt den **silent persistence-killer**: wenn ein Page-Builder einen audit-Pfad referenziert der nicht im Catalog ist, rendert das PDF korrekt (Backend liest direkt), aber Editor zeigt "(statisch)" — und User-Save zerstoert das Audit-Binding. **In M13 erweitert** (vorher nur `sections.*`-Prefix gepruerft): jetzt deckt der Hook ALLE audit-Pfade ab (auch `comparison.*`, `phasenplan.*`, `summary.*`, `inhaber.*`, `topRisks`, `recommendations`, `screenshots.*`) UND **Index-Pfade** in template-strings. Beispiel: `path: \`summary.topIssues[${"$"}{idx}].headline\`` wird zu Regex `summary\\.topIssues\\[[a-zA-Z0-9_]+\\]\\.headline` und gegen Catalog gematcht — ${"$"}{...} matcht numerische Indices wie `[0]` UND string-keys wie `.html`. Wenn 0 Catalog-Match: Hook-WARN. Hat den M13-topIssues-Bug bei Build-Time gefangen (haette 2h E2E-Diagnose gespart) und entdeckt aktuell den latenten M5.1-Bug fuer altSentences-Index-Pfade. Output: Liste fehlender Paths oder ✓-Counter. Skript: `.claude/hooks/binding-catalog-consistency.sh`.
